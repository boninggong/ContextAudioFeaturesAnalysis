...
for j, act in enumerate(time_of_day):
...
def radar_plot():
    ...
    values += values[:1]
    ax.plot(angles, values, linewidth=1, linestyle='solid', label="Morning")
    ax.fill(angles, values, 'b', alpha=0.1)

    # Ind2
    values = df.loc[1].drop('group').values.flatten().tolist()
    values += values[:1]
    ax.plot(angles, values, linewidth=1, linestyle='solid', label="Afternoon")
    ax.fill(angles, values, 'r', alpha=0.1)

    # Ind1
    values = df.loc[2].drop('group').values.flatten().tolist()
    values += values[:1]
    ax.plot(angles, values, linewidth=1, linestyle='solid', label="Evening")
    ax.fill(angles, values, 'g', alpha=0.1)

    # Ind2
    values = df.loc[3].drop('group').values.flatten().tolist()
    values += values[:1]
    ax.plot(angles, values, linewidth=1, linestyle='solid', label="Night")
...
def line_plot():
    plt.plot([1, 2, 3, 4, 5, 6, 7, 8, 9], time_of_day_values[0], 'b', label='Morning')
    plt.plot([1, 2, 3, 4, 5, 6, 7, 8, 9], time_of_day_values[1], 'r', label='Afternoon')
    plt.plot([1, 2, 3, 4, 5, 6, 7, 8, 9], time_of_day_values[2], 'g', label='Evening')
    plt.plot([1, 2, 3, 4, 5, 6, 7, 8, 9], time_of_day_values[3], 'm', label='Night')
    plt.axis([1, 9, 0, 1])
    plt.ylabel('Values')
