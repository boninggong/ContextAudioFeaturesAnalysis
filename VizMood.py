...
moods_values = [[] for _ in range(2)]

for j, act in enumerate(moods):
    dat = pd.read_csv(f"audio_features/audio_features_{act}.csv")[:1].values[0]
    for i, feat in enumerate(wanted_features):
        moods_values[j].append(dat[AUDIO_FEATURES.index(feat)])
...
def radar_plot():
    ...
    values += values[:1]
    ax.plot(angles, values, linewidth=1, linestyle='solid', label="Happy")
    ax.fill(angles, values, 'b', alpha=0.1)

    # Ind2
    values = df.loc[1].drop('group').values.flatten().tolist()
    values += values[:1]
    ax.plot(angles, values, linewidth=1, linestyle='solid', label="Sad")
...
def line_plot():
    plt.plot([1, 2, 3, 4, 5, 6, 7, 8, 9], moods_values[0], 'b', label='Happy')
    plt.plot([1, 2, 3, 4, 5, 6, 7, 8, 9], moods_values[1], 'r', label='Sad')
    plt.axis([1, 9, 0, 1])
    plt.ylabel('Values')
